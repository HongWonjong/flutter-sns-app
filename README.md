...
이 프로젝트는 Flutter를 사용한 간단한 SNS 앱으로, 클린 아키텍처와 `flutter_riverpod`를 기반으로 설계되었습니다. 사용자는 게시물을 작성하고, 목록을 무한 스크롤로 조회하며, 댓글을 작성/조회할 수 있습니다. 앱 시작 시 스플래시 화면이 표시되며, 게시물 작성 시 TensorFlow Lite와 YOLO 모델을 사용해 이미지를 검사하여 사람 객체가 포함된 이미지는 업로드가 차단됩니다. Firebase Firestore와 Firebase Storage를 데이터베이스로 사용합니다.

---

## 🚀 기능

- **스플래시 화면**: 앱 시작 시 로딩 화면 표시, Firebase 초기화 및 초기 데이터 로드.
- **게시물 목록 조회**: 무한 스크롤로 게시물 목록 표시.
- **게시물 작성**: 텍스트와 이미지를 포함한 게시물 작성, 이미지 업로드 전 YOLO 모델로 사람 객체 검사.
- **댓글 보기/작성**: 특정 게시물의 댓글 목록 조회 및 댓글 작성.
- **Firebase 연동**: Firestore로 데이터 저장, Storage로 이미지 저장.
...
│   │       └── app_bar.dart           # 공통 AppBar: 뒤로가기 버튼, 제목, 연필 버튼
│   │       └── bottom_bar.dart        # 공통 하단 바: 새로고침, 글자 수 카운터, 페이지 이동 버튼
